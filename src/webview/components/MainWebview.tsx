...
const BySprintsView: FC<PortfolioViewProps> = ({
	...
	const additionalTabContent = selectedUserStory
		? {
				tasks: <TasksTable tasks={tasks as any} loading={tasksLoading} error={tasksError} onLoadTasks={() => selectedUserStory && onLoadTasks(selectedUserStory.objectId)} embedded />,
				tests: <TestCasesTable testCases={userStoryTestCases} loading={userStoryTestCasesLoading} error={userStoryTestCasesError || undefined} embedded />,
				defects: (
					<DefectsTable
...
const MainWebview: FC<MainWebviewProps> = ({ webviewId, context, _rebusLogoUri }) => {
	...
	useEffect(() => {
		if (tabFadeKey === displayedKey) return;
		pendingKeyRef.current = tabFadeKey;
		setIsLeaving(true);
		...
		const handleMessage = (event: MessageEvent) => {
			...
			switch (message.command) {
				...
				case 'holidaysError':
					...
					break;
				case 'customEventsLoaded':
					setCustomCalendarEvents(message.events || []);
					break;
				case 'customEventSaved':
