...
type Section = 'search' | 'home' | 'portfolio' | 'team' | 'library' | 'metrics' | 'collaboration' | 'test';
...
const NavigationBar: React.FC<NavigationBarProps> = ({ activeSection, onSectionChange, collaborationBadgeCount = 0, showTestTab = false }) => {
	...
	const tabs = useMemo(() => {
		...
		// Insert before Collaboration/Library so Test is easier to find in the overflow menu
		const testTab = { id: 'test' as const, label: 'Test', Icon: TestIcon, iconOnly: false };
